...
class SongController {
  static createSong = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        res
          .status(201)
          .json({ message: "Song created successfully", data: req.body });
      } catch (error) {
        return next(error);
      }
    }
  ...
  static getArtistAllSongs = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  static searchedSongs = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      ...
      try {
        const searchedSongs = await SongService.getSearchedSongs(
          query.toString()
        );
        if (!searchedSongs || searchedSongs.length === 0) {
          return res
            .status(404)
            .json({ message: "No songs match your search" });
        }
  ...
  static searchedSongsById = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      ...
      try {
        const searchedSongs = await SongService.getSearchedSongsById(
          query.toString()
        );
        if (!searchedSongs || searchedSongs.length === 0) {
          return res
